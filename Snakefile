...
OUTPUT_FILES.extend(expand(result("{name}_variants_merged_sorted.vcf"), name=list(set(INPUT_FILES))))
...
rule filter_variants:
    input: "vcf_base/{name}_merged_base.vcf"
    output: "vcf_filtered/{name}_merged_filtered.vcf"
    run:
...
rule split_complex_variants:
    input: "vcf_filtered/{name}_merged_filtered.vcf"
    output: "vcf_prim/{name}_merged_prim.vcf"
    run:
        shell("vcfallelicprimitives -kg {input} > {output}")

rule vcffixup:
    input: "vcf_prim/{name}_merged_prim.vcf"
    output: "vcf_prim/{name}_merged_prim_fixed.vcf"
    run:
...
rule split_multiallelic_variants:
    input: "vcf_prim/{name}_merged_prim_fixed.vcf"
    output: "vcf_split/{name}_merged_split.vcf"
    run:
...
rule re_align_INDELS:
    input: "vcf_split/{name}_merged_split.vcf"
    output: "vcf_aligned/{name}_merged_aligned.vcf"
    run:
...
rule sort_variants:
    input: "vcf_aligned/{name}_merged_aligned.vcf"
    output: result("{name}_variants_merged_sorted.vcf")
    run:
